...
internals.loadDocClient = function () {
  internals.docClient = internals.docClient || DynamoDBDocumentClient.from(internals.loadDynamoClient());
  return internals.docClient;
};
...
internals.compileModel = function (name, schema) {
  ...
  Model.config = function(config) {
    ...
    if (config.dynamoClient) {
      table.docClient = DynamoDBDocumentClient.from(config.dynamoClient);

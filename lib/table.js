...
var _            = require('lodash'),
    ...
    { CreateTableCommand, DeleteTableCommand, DescribeTableCommand, UpdateTableCommand } = require('@aws-sdk/client-dynamodb'),
...
var Table = module.exports = function (name, schema, serializer, docClient, dynamoClient, logger) {
...
internals.createItem = function (table, item, options, callback) {
  ...
  internals.callBeforeHooks(self, 'create', start, function (err, data) {
    ...
    fs.writeFileSync('item.json', JSON.stringify(params, null, 2));
    ...
    fs.writeFileSync('item1.json', JSON.stringify(params, null, 2));

    const fakeParams = {
      TableName: 'dynamo-int-test-users',
      Item: marshall({
        id: '123456789',
        email: 'some@user.com',
      })
    ...
    self.docClient.send(new PutCommand(fakeParams))
      .then(() => {
...
Table.prototype.createTable = function (options, callback) {
  ...
  if (_.isFunction(_callback)) {
    this.createTableAsync(options)
      .then(data => _callback(null, data))
      .catch(err => _callback(err, null)); 
  } else {
    return this.createTableAsync(options);
  }
};

Table.prototype.createTableAsync = function (options = {}) {
  const self = this;
  ...
  return self.dynamoClient.send(new CreateTableCommand(params));
};

Table.prototype.describeTable = function (callback) {
  if (_.isFunction(callback)) {
    this.describeTableAsync()
      .then(data => callback(data !== null, data))
      .catch(err => callback(err, null)); 
  } else {
...
Table.prototype.describeTableAsync = async function () {
  const params = {
    TableName : this.tableName(),
  };
  try {
    const data = await this.dynamoClient.send(new DescribeTableCommand(params));
    return data;
  } catch (err) {
    return null;
  }
...
Table.prototype.deleteTableAsync = async function () {
  var params = {
    TableName : this.tableName(),
  };
  return this.dynamoClient.send(new DeleteTableCommand(params));
};

Table.prototype.updateTable = function (throughput, callback) {
  if (_.isFunction(callback)) {
    this.updateTableAsync(throughput)
      .then(data => callback(null, data))
      .catch(err => callback(err, null)); 
  } else {
    return this.updateTableAsync(throughput);
  }
};

Table.prototype.updateTableAsync = async function (throughput = {}) {
  const self = this;
  return Promise.all([
    internals.syncIndexes(self, throughput),
    internals.updateTableCapacity(self, throughput),
  ]);
};

internals.updateTableCapacity = async function (table, throughput) {
  var params = {};
  ...
  if (!_.isEmpty(params)) {
    ...
    return table.dynamoClient.send(new UpdateTableCommand(params));
  } else {
    return Promise.resolve(true);
  }
};

internals.syncIndexes = async function (table) {
  const data = await table.describeTable();
  ...
  for (let i = 0; i < missing.length && i < maxIndexCreationsAtaTime; i++) {
    const params = missing[i];
    const attributeDefinitions = [];
    ...
    await table.dynamoClient.send(new UpdateTableCommand(updateParams));
  }
  return true;
};
